...
export default function DepositPage() {
  ...
  const handleDeposit = async (e: React.FormEvent) => {
    ...
    if (!token) {
      setError('Vous devez être connecté pour effectuer un dépôt');
      return;
    }
    ...
    try {
      ...
      if (response && (response._id || response.id)) {
        ...
        setError('Échec de la demande de dépôt');
      }
    } catch {
      setError('Une erreur est survenue lors de la demande de dépôt');
    } finally {
      setIsLoading(false);
    }
  };
          ...
          <div>
            <div className="flex items-center space-x-3 mb-6">
              ...
              <h1 className="text-xl font-black text-white uppercase tracking-widest">
                Recharge du <span className="gold-text">Portefeuille</span>
              </h1>
            </div>
            <p className="text-gray-500 font-medium tracking-tight">Alimentez la station en crédits pour commencer votre session gagnante.</p>
          </div>
        </div>
                  ...
                  <form onSubmit={handleDeposit} className="space-y-10">
                    <div>
                      <div className="flex items-center justify-between mb-6 px-1">
                         <label className="block text-[10px] font-black text-gray-600 uppercase tracking-[0.3em]">Montant du Dépôt (TND)</label>
                      </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                      {[20, 50, 100, 500].map((quick) => (
                        <button
                          ...
                          className="bg-white/5 border border-white/5 rounded-2xl py-4 px-2 font-black text-xs uppercase tracking-widest transition-all text-gray-500 hover:text-white hover:bg-white/10"
                        >
                          {quick} TND
                        </button>
                      ))}
                    ...
                    <button
                      ...
                      className="group w-full inline-flex items-center justify-between pl-10 pr-2 py-2 bg-[#d3a936] rounded-full disabled:opacity-50 mt-4 transition-all hover:scale-[1.01]"
                    >
                      <span className="font-black uppercase text-[12px] text-black tracking-[0.4em]">
                        {isLoading ? 'Demande en cours...' : `Déposer ${amount || '0.0'} TND`}
                      </span>
                      <div className="w-14 h-14 bg-black/10 rounded-full flex items-center justify-center transform group-hover:scale-110 transition-transform">
             ...
             <div className="bezel-shell p-1 flex-1">
                <div className="bezel-core bg-black rounded-[2rem] p-8 h-full flex flex-col justify-center">
                   <div className="flex items-center space-x-3 mb-6">
                      ...
                      <span className="text-gray-600 font-black uppercase text-[10px] tracking-widest">Crédits Totaux</span>
                   </div>
             ...
             <div className="bezel-shell p-1 flex-1">
                <div className="bezel-core bg-black rounded-[2rem] p-8 h-full flex flex-col justify-center">
                   <div className="flex items-center space-x-3 mb-8">
                      ...
                      <h3 className="text-white font-black uppercase text-[10px] tracking-widest">Infos Recharge</h3>
                   </div>
                   <ul className="space-y-6">
                      {[
                        'Soumettez la demande et contactez l\'administrateur.',
                        'Les bonus standards sont appliqués automatiquement à l\'approbation.',
                        'Une fois autorisés, les crédits apparaîtront instantanément.'
                      ].map((text, i) => (
