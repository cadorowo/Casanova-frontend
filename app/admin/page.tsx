...
export default function AdminPage() {
  ...
  const [clientTimeFilter, setClientTimeFilter] = useState('7d');
  const [masterTimeFilter, setMasterTimeFilter] = useState('7d');
  ...
  const fetchAdminData = useCallback(async () => {
    if (!token) return;
    const days1 = parseInt(clientTimeFilter.replace('d', '')) || 7;
    const days2 = parseInt(masterTimeFilter.replace('d', '')) || 7;
    const maxDays = Math.max(days1, days2);
    try {
      const [logs, users, pending] = await Promise.all([
        api.admin.getFinancialLogs(token, maxDays),
  ...
  }, [token, clientTimeFilter, masterTimeFilter]);

  useEffect(() => {
    if (token) {
      const timer = setTimeout(async () => {
        try {
          const users = await api.admin.getUsers(token, searchTerm);
          setUsersList(users?.success ? users.data : users);
        } catch (e) {}
      }, 300);
  ...
  const getTxConfig = (type: string) => {
    switch (type.toLowerCase()) {
      case 'deposit': return { icon: Plus, label: 'Dépôt', color: 'text-white', bg: 'bg-purple-600/10' };
      case 'withdrawal': return { icon: ArrowDown, label: 'Retrait', color: 'text-white', bg: 'bg-gray-500/10' };
        ...
        <div className="mb-10 lg:mb-16">
         ...
         <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 mb-10 lg:mb-16 items-stretch">
              ...
              <div className="bezel-core cassanova-glass p-6 md:p-8 flex flex-col justify-between rounded-[2rem] border border-yellow-500/20 h-full">
                 <div className="flex justify-between items-start mb-6">
                    <div className="flex items-center space-x-4">
                       <div className="w-12 h-12 bg-[#f2d335]/10 rounded-xl flex items-center justify-center border border-[#f2d335]/20 shadow-xl shrink-0">
                          <Banknote size={28} className="text-[#f2d335]" />
                       </div>
            ...
            <div className="xl:col-span-8 animate-reveal flex">
               <div className="bezel-shell p-1 w-full shadow-2xl h-full">
                  <div className="bezel-core bg-black rounded-[var(--cassanova-radius)] overflow-hidden flex flex-col h-full min-h-[320px]">
                     <div className="p-5 md:p-6 border-b border-white/5 bg-white/[0.02] flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div className="flex items-center space-x-4 shrink-0">
                           ...
                           <h2 className="text-xl font-black text-white uppercase tracking-tighter">Gestion de la File d&apos;Attente</h2>
                        ...
                        <div className="w-full sm:w-80 self-start sm:self-center bg-white/5 rounded-full px-5 py-2.5 flex items-center space-x-3 border border-white/5 hover:border-blue-600/30 transition-all group">
                                 ...
                                 <div key={req._id} className="cassanova-row-slim min-h-[60px] md:min-h-0 group mb-1 last:mb-0 py-2 md:py-1">
                                    <div className="flex items-center space-x-4 flex-1 min-w-0">
                                       <div className={`w-6 h-6 rounded flex items-center justify-center shrink-0 ${
                                         req.type === 'deposit' ? 'bg-purple-600 text-white' : 'bg-gray-500 text-white'
                                       }`}>
                                          {req.type === 'deposit' ? <Plus size={12} /> : <ArrowDown size={12} />}
                                       </div>
                                       ...
                                       <div className="flex flex-1 min-w-0 items-center justify-between md:justify-start md:space-x-8 pr-2">
                                          ...
                                          <div className="flex flex-col md:flex-row md:items-center min-w-0 md:space-x-8 flex-1">
                                             <div className="md:w-[200px] flex flex-row items-center space-x-2 min-w-0 shrink-0">
                                                <span className="hidden md:inline text-sm md:text-base font-black uppercase tracking-tighter truncate text-white">
                                                   {req.type === 'deposit' ? 'Déposer' : 'Retirer'}
                                                </span>
                                                <Link 
                                             ...
                                             <div className="hidden md:flex items-center flex-1">
                                                ...
                                                <div className="flex-1 text-right text-base font-black text-white tracking-tighter whitespace-nowrap pr-4">
                                                   {req.type === 'withdrawal' ? '-' : '+'}{req.amount.toFixed(1)} <span className="text-[10px] opacity-40 uppercase">TND</span>
                                                </div>
                                             </div>
                                          </div>
               ...
               <div className="bezel-core bg-black rounded-[var(--cassanova-radius)] overflow-hidden flex flex-col">
                  <div className="p-5 md:p-6 border-b border-white/5 bg-white/[0.02] flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                     <div className="flex items-center space-x-4">
                        <div className="w-1 h-6 bg-blue-600 rounded-full" />
                        <div className="flex items-center space-x-4">
                           <h2 className="text-xl font-black text-white uppercase tracking-tighter">Annuaire des Joueurs</h2>
                           <div className="flex items-center space-x-2">
                   ...
                   <div className="p-3 md:p-4 grid grid-cols-1 lg:grid-cols-2 gap-2 md:max-h-[600px] md:overflow-y-auto custom-scrollbar">
                     {paginatedUsers.map((player) => (
                        <div key={player.id || player._id} className="cassanova-row-slim min-h-[50px] md:min-h-0 group">
                           <div className="flex items-center flex-1 min-w-0">
                              <div className="flex items-center space-x-6 min-w-0 px-2">
                                 <Link 
                                    href={`/admin/credits?phone=${player.phone}`}
                                    className="w-24 md:w-28 text-base font-black text-blue-600 hover:text-blue-500 tracking-tighter truncate shrink-0 transition-colors tabular-nums"
                                 >
                                    {player.phone}
                                 </Link>
                     ...
                     {filteredUsers.length === 0 && (
                       <div className="lg:col-span-2 py-24 text-center opacity-20">
                          <p className="text-[10px] font-black text-gray-500 uppercase tracking-[0.4em]">Aucun Personnel Trouvé</p>
                       </div>
                     )}
                     {totalUsersPages > 1 && (
                       <div className="lg:col-span-2 flex items-center justify-between pt-4 border-t border-white/5 mt-4 shrink-0">
                          <span className="text-[9px] font-black text-gray-600 uppercase tracking-widest">
                ...
                <div className="bezel-core bg-black rounded-[var(--cassanova-radius)] overflow-hidden flex flex-col">
                   ...
                   <div className="p-5 md:p-6 border-b border-white/5 bg-white/[0.02] flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      <div className="flex items-center space-x-4">
                         ...
                         <h2 className="text-xl font-black text-white uppercase tracking-tighter">DISTRIBUTION CLIENTS</h2>
                      </div>
                    ...
                    {}
                     <div className="p-3 md:p-6 flex-1 flex flex-col md:overflow-y-auto no-scrollbar md:max-h-[600px]">
                       {(() => {
                         ...
                         return (
                            <>
                               <div className="space-y-2 md:max-h-[550px] md:overflow-y-auto custom-scrollbar pr-2">
                                  {paginatedTxs.map((tx) => {
                                     ...
                                     if (tx.type.toLowerCase() === 'deposit' || tx.type.toLowerCase() === 'admin_transfer') {
                                        ...
                                        displayBg = 'bg-gray-500';
                                        DisplayIcon = ArrowDown;
                                     }

                                     return (
                                        <div key={tx._id} className="cassanova-row-slim min-h-[60px] md:min-h-0 group mb-1 last:mb-0 py-2 md:py-1">
                                           <div className="flex items-center space-x-4 flex-1 min-w-0">
                                              ...
                                              <div className="flex flex-1 min-w-0 items-center justify-between md:justify-start pr-2">
                                                 <div className="flex flex-col md:flex-row md:items-center min-w-0 flex-1 gap-1 md:gap-0">
                                                    <div className="md:w-[200px] flex items-center space-x-2 min-w-0 shrink-0">
                                                       <span className={`text-sm md:text-base font-black uppercase tracking-tighter truncate ${displayColor} ${tx.type === 'admin_transfer' ? 'hidden md:inline' : ''}`}>
                                                          {config.label === 'Master Buy-In' || config.label === 'Distributed' || config.label === 'Distribué' ? <span className="hidden md:inline">{config.label}</span> : config.label}
                                                       </span>
                                                    ...
                                                    <div className="md:hidden text-base font-black text-white tracking-tighter">
                                                       {tx.type === 'withdrawal' ? '-' : '+'}{tx.amount.toFixed(1)} <span className="text-[10px] opacity-40 uppercase">TND</span>
                                                    </div>
                                                 ...
                                                 <div className="md:hidden flex flex-col items-end space-y-0.5 shrink-0 ml-auto">
                                                    <span className="text-[8px] font-black text-gray-600 uppercase tracking-widest">
                                        ...
                                        <div key={tx._id} className="cassanova-row-slim min-h-[60px] md:min-h-0 group mb-1 last:mb-0 py-2 md:py-1">
                                           <div className="flex items-center space-x-4 flex-1 min-w-0">
                                              ...
                                              <div className="flex flex-1 min-w-0 items-center justify-between md:justify-start pr-2">
                                                 <div className="flex flex-col md:flex-row md:items-center min-w-0 flex-1 gap-1 md:gap-0">
                                                    <div className="md:w-[200px] flex items-center space-x-2 min-w-0 shrink-0">
                                                       <span className={`text-sm md:text-base font-black uppercase tracking-tighter truncate ${displayColor}`}>
                                                          {config.label === 'Master Buy-In' || config.label === 'Distributed' ? <span className="hidden md:inline">{config.label}</span> : config.label}
                                                       </span>
                                                    </div>
                                                    
                                                    <div className="md:hidden text-base font-black text-white tracking-tighter">
                                                       +{tx.amount.toFixed(1)} <span className="text-[10px] opacity-40 uppercase">TND</span>
                                                    </div>
                                                 ...
                                                 <div className={`w-8 h-6 md:w-auto px-2 py-0.5 rounded-full ${statusCfg.bg} border ${statusCfg.border} ${statusCfg.color} text-[7px] font-black uppercase tracking-widest flex items-center justify-center space-x-1 shrink-0`}>
